...
const playlistFileName = "bgmfoxPlaylists.rdf";
...
function onSearchResultDblClicked(event) {
	var selectedVideo = event.currentTarget.selectedItem;
	var video = newVideo(selectedVideo.label, selectedVideo.value);
...
function VideoManager(videoSites) {
	...
	this.loadVideo = function(video) {
		...
		if (this.isNicoVideoId(video.get_id())) {
		...
		document.getElementById("mrqTitle").textContent = video.get_title();
	...
	this.checkPrevNextVideo = function() {
		if (this.currentPlaylist === null || this.currentPlaylist === undefined) {
	...
	this.checkRepeatState = function() {
		if (document.getElementById("imgNoRepeat").style.display === "block") {
			this.repeatValue = enumRepeatValues.noRepeat;
		} else if (document.getElementById("imgAllRepeat").style.display === "block") {
	...
	this.isNicoVideoId = function(videoId) {
		return videoId.match(/sm\d*/);
	};
	...
	this.openOriginalSite = function() {
		...
		if (this.isNicoVideoId(selectedVideoId)) {
...
function NiconicoProto() {
	...
	this.login = function(force) {
		...
		var exePath = ext.path + "\\chrome\\content\\NiconicoUtility.exe";
		...
		cookieFile.initWithPath(ext.path + "\\chrome\\content\\cookie.txt");
	...
	this.isLoggedIn = function(force) {
...
NiconicoProto.searchNicoVideo = function() {
	...
	for (let i = 1; i < videoManager.searchResultsPerPage + 1; i++) {
		...
		for (var j = 0; j < paragraphs.length; j++) {
			if (paragraphs[j].getAttribute("class") == "TXT12") {
...
var createPlaylistsManager = function () {
	...
	that.loadedFunc = function() {
		let treecols = trPlContents.firstChild.childNodes;
		for (let i = 0; i < (treecols.length * 2) - 1; i = i + 2) {
			if (treecols[i].getAttribute("sortActive") === "true") {
				...
				sortService.sort(trPlContents, treecols[i].getAttribute("sort"), treecols[i].getAttribute("sortDirection"));
				break;
			}
		}
	...
	var _init = function() {
		if (trPlContents.getAttribute("style").indexOf("none") === -1) {
	...
	var _flushDataSource = function (doRebuild) {
		dsource.QueryInterface(Components.interfaces.nsIRDFRemoteDataSource).Flush();
		if (doRebuild && rlbPlaylists.selectedItem !== null) {
			_rebuildPlaylist();
		}
	};
	...
	var _rebuildPlaylist = function(func) {
		if (rlbPlaylists.selectedItem.tagName !== "richlistitem") {
			log("playlists.selectedItem.tagName is:" + rlbPlaylists.selectedItem.tagName);
			_rebuildPlaylists();
			return;
		}
	...
	that.deletePlaylist = function (isRename) {
		...
		if (name === "Library" || selectedPlaylist.isMaster()) {
			return;
		}
...
//			);
	...
	var clearPlaylistContent = function() {
		var tree = document.getElementById("trPlaylistContents");
		if (tree.view !== null) {
			tree.view.selection.clearSelection();
			while (tree.rowCount > 0) {
				tree.view.performActionOnRow("delete", 0);
			}
		}
	};
	...
	var _getVideoInfo = function(videoId, callback) {
		...
		if (videoManager.isNicoVideoId(videoId)) {
	...
	var _writeVideoResource = function(newnode, index, videoInfo, playlistTitle) {
		...
		let tbbxMainList = document.getElementById("tbbxMainList");
		let oldTabIndex = tbbxMainList.selectedIndex;
		let oldPlaylistIndex = rlbPlaylists.selectedIndex;
		_flushDataSource();
	};
	...
	that.playlistContentObserver = function(event) {
		...
		case "dragover":
			...
			try {
				targetIndex = trPlContents.boxObject.getRowAt(event.clientX, event.clientY);
			} catch (e) {
				return;
			}
			...
			var tHeight = trPlContents.boxObject.rowHeight;  // target height
			var headerHeight = 24;
			var tY = trPlContents.boxObject.y + headerHeight + targetIndex * tHeight;  // target y pos
			var mY = event.clientY;  // mouse y pos
			var tCenterY = tY + tHeight / 2;
...
//			rlbPlaylists.selectedIndex = oldIndex;
	...
	that.onPlaylistsPanelKeyup = function(event) {
		...
		} else if (event.ctrlKey  && event.keyCode === event.DOM_VK_RIGHT) {
			videoManager.nextVideo();
		} else if (event.ctrlKey && event.keyCode === event.DOM_VK_LEFT) {
			videoManager.previousVideo();
		} else if (event.shiftKey && event.altKey && event.keyCode === event.DOM_VK_N) {
			playlistsManager.createPlaylist();
		}
	};
	
	that.onPlayListItemDblClicked = function(event) {
		var video;
		
		if (event.currentTarget.parentNode.id === "trPlaylistContents") {
		...
		} else if (event.currentTarget.tagName === "vbox") {
			video = selectedPlaylist.get(gridView.get_selectedVideoId());
		} else {
			throw "invalid target: " + event.currentTarget.tagName;
		}
			
		if (event.currentTarget.parentNode.id == "trPlaylistContents" || event.target.tagName == "image") {
	...
	that.onTooltipShowing = function(event) {
		let row = {}, column = {}, part = {};
		try {
			trPlContents.boxObject.getCellAt(event.clientX, event.clientY, row, column, part);
		} catch(e) {
//			event.preventDefault();
		}
...
var newPlaylist = function (name) {
	...
	that.hasNextOf = function(video) {
		...
		if (repeatValue == enumRepeatValues.noRepeat) {
			return indexOf(video) < (items.length - 1);
...
var createRdfManager = function() {
	...
	that.importLibrary = function() {
		...
		fp.appendFilter("BGM Fox Library file","*.rdf");
		var res = fp.show();
		if (res == nsIFilePicker.returnOK){
					...
					setTimeout(
						function() {
							rdfManager.loadDataSource(
								function() {
									document.getElementById("rlbPlaylists").builder.rebuild();
									document.getElementById("addToPlaylist-context").builder.rebuild();
