var bgmfoxGlobal = {}; // グローバル名前空間オブジェクト
with (bgmfoxGlobal) {
	let ns = bgmfoxGlobal;
	...
	//
	////////////////////////////////////////////////////////////////////////////////////////////////////
	ns.Cc = Components.classes;
	ns.Ci = Components.interfaces;
	ns.playlistFileName = "bgmfox.rdf";
	ns.regExprNicoVideoId = /^(\w\w\d+)$|^(\d+)$/;
	ns.loginHostName = "https://secure.nicovideo.jp";
	...
	ns.nsLoginInfo = new Components.Constructor("@mozilla.org/login-manager/loginInfo;1",
			Components.interfaces.nsILoginInfo,
			"init");
	ns.prefBranch = prefSvc.getBranch("extensions.bgmfox.");
	ns.lastSelectedTabIndex = -1;
	...
	ns.onLoad = function() {
		try {
			...
			ns.playlistsManager = createPlaylistsManager();
			ns.rdfManager = createRdfManager();
			...
			ns.videoManager = new VideoManager({ youtube: youtube, niconico: niconico });
			videoManager.init();
			mylistManager.init();
			...
			ns.scrollingVideoTitle = newScrollingVideoTitle();

			var selectedIndex = document.getElementById("tbplsMainList").selectedIndex;
			if (selectedIndex == enumTabValues.mylist) {
				ns.gridView = mylistManager.gridView;
				ns.currentPlaylistManager = mylistManager;
				mylistManager.loadMylists();
			} else {
				ns.gridView = playlistsManager.gridView;
				ns.currentPlaylistManager = playlistsManager;
			}
	...
	ns.isNicoVideoId = function(videoId) {
		return videoId.match(regExprNicoVideoId);
	};

	ns.getVideoInfo = function(videoId, callback) {
		if (!videoId) {
	...
	ns.onTabSelected = function(event) {
		...
		if (ns.playlistsManager && mylistManager) {
			if (selectedIndex == enumTabValues.mylist) {
	...
	ns.onImgMuteMouseOut = function(event) {
		if ((event.target.boxObject.y + event.target.boxObject.height) > event.clientY) {
			document.getElementById("popupVolume").hidePopup();
		}
	};

	ns.onSearchResultDblClicked = function(event) {
	...
	ns.onMarqueeClick = function(event) {
		event.target.stop();
	};

	ns.onMarqueeDblClick = function(event) {
		event.target.start();
	};

	ns.onLblTimeClick = function(event) {
	...
	//
	//////////////////////////////////////////////////////////////////
	ns.sendLoginQuery = function(mail, password) {
	...
	ns.joinGroupIds = function(groupIds) {
		return _joinIds(groupIds, "group_id_list%5B%5D=")
	};

	ns._joinIds = function(ids, str) {
		var result = "";
		for (i in ids) {
			result += str + ids[i] + "&";
		}
	...
	ns.secondsToStr = function(seconds) {
	...
	ns.strToSeconds = function(string) {
		var minutes;
		var seconds;
		var regExpr = /(\d+):(\d+)/;
		var rObj = new RegExp(regExpr);
	...
	ns.writeStatusbar = function(message) {
	 ...
	 */
	ns.VideoManager = function(videoSites) {
		// Fields
			...
			switch (stateValue) {
				...
				case this.enumPlayerStates.load:
					...
					if (videoManager.isPlaylistItemPlaying && !videoManager.currentPlaylist.isMylist) {
						setTimeout(function() {
							playlistsManager.updateVideoInfo(videoManager.playingVideo.get_id());
						}, 5000);
					}
	...
	ns.VideoSite = function(htmlDocument) {
		this.initialize.apply(this, arguments);
	};

	ns.VideoSite.prototype = {
		videoId: "",
		htmlDocument: "",
		...
		load: function(videoId) {
			...
			setTimeout(function (htmlDocument) {
				htmlDocument.getElementById("btnChangeVideoSite").click();
			}, 100, this.htmlDocument);
		},
	...
	ns.Youtube = function(htmlDocument) {
		this.initialize.apply(this, arguments);
	};
	...
	ns.YoutubeProto.prototype = new VideoSite();
	ns.Youtube.prototype = new YoutubeProto();

	ns.Niconico = function(htmlDocument) {
	...
	ns.NiconicoProto.searchNicoVideo = function(niconicoProto, query, page) {
	...
	ns.NiconicoProto.prototype = new VideoSite();
	ns.Niconico.prototype = new NiconicoProto();

	/**
	 * @class PlaylistsManager
	 */
	ns.createPlaylistsManager = function() {
		var that = {};
		...
		that.deletePlaylist = function (isRename) {
			if (!prompts.confirm(window, "Remove", "remove playlist ?")) {
				return;
			}
 ...
 */
	...
	ns.mylistManager = {
		...
		deleteMylist: function () {
			if (!prompts.confirm(window, "Remove", "remove mylist ?")) {
				return;
			}
		...
		onPlaylistSelectionChanged: function() {
			...
			$.get(query, function(json_response) {
				...
				for (let i = 0; i < mylistItems.length; i += 1) {
					...
					lblTitle.addEventListener("click", function(event) {
