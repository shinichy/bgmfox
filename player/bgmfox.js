...
function onPlayerStateChange(newState) {
	...
	case 2:
		newState = 2;
		// nico api fires paused event when loaded...
		if (!playerManager.isYoutube && !playerManager.isNicoVideoLoaded) {
			playerManager.isNicoVideoLoaded = true;
			onPlayerStateChange("load");
		}
...
//			player.ext_setVideoSize("fit");
	...
	that.updatePlayerStatus = function() {
		try {
			...
			} else if (newState == "stopped" && !playerManager.isNicoVideoLoaded) {
				throw("error");
			}
		} catch(e) {
			errorCount++;
	...
	that.load = function(videoId, func) {
		var so = new SWFObject("http://res.nimg.jp/swf/player/nicoplayer.swf?ts=1265087081", "flvplayer", "952", "512", 9, "#FFFFFF");
